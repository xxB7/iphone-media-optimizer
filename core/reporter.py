...
class ExecutionReporter:
    ...
    def display_terminal_summary(self, stats: Dict[str, Any], elapsed_seconds: float):
        ...
        table = Table(title="📊 تقرير إنجاز تخفيف استديو الآيفون (iPhone DCIM Summary)", show_header=True, header_style="bold magenta")
        table.add_column("المعيار / Metric", style="cyan", width=32)
        table.add_column("القيمة / Value", style="bold green", width=24)

        table.add_row("إجمالي الملفات (Total Files)", str(stats.get("total_count", 0)))
        table.add_row("تم ضغطها بنجاح (Compressed)", str(stats.get("success_count", 0)))
        table.add_row("تم الإبقاء على الأصل (Kept Original)", str(stats.get("skipped_larger_count", 0)))
        table.add_row("ملفات مساندة منسوخة (Sidecars)", str(stats.get("copied_count", 0)))
        table.add_row("ملفات واجهت أخطاء (Failed)", str(stats.get("failed_count", 0)))
        table.add_section()
        table.add_row("الحجم الأصلي (Original Size)", format_bytes(orig_bytes))
        table.add_row("الحجم بعد المعالجة (New Size)", format_bytes(comp_bytes))
        table.add_row("المساحة الموفرة (Space Saved)", f"[bold yellow]{format_bytes(saved_bytes)}[/bold yellow]")
        table.add_row("نسبة التخفيض (Reduction Ratio)", f"[bold green]{ratio:.1f}%[/bold green]")
        table.add_section()
        mins, secs = divmod(int(elapsed_seconds), 60)
        table.add_row("وقت المعالجة الإجمالي (Duration)", f"{mins}m {secs}s")
